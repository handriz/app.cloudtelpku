...
<div id="rbm-workspace" class="flex flex-col h-[calc(100vh-80px)] bg-gray-100 dark:bg-gray-900 transition-all duration-300 group/workspace">
    ...
    <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between shadow-sm z-20 shrink-0">
        ...
        {{-- PAPAN INFO POSISI KODE (Sticky) --}}
        <div class="absolute left-1/2 transform -translate-x-1/2 top-14 z-30 pointer-events-none">
            <div class="bg-gray-800 text-white px-6 py-2 rounded-b-xl shadow-lg flex items-center space-x-3 pointer-events-auto" id="kddk-context-bar">
                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">Posisi:</span>
                <span id="live-kddk-display" class="text-xl font-mono font-bold tracking-widest text-yellow-400">
                    {{ $basePrefix }}
                </span>
            </div>
        </div>
    <meta name="csrf-token" content="{{ csrf_token() }}">
        ...
        <div class="flex items-center space-x-2">
            ...
            {{-- TOMBOL TOGGLE MAP (BARU) --}}
            <button type="button" data-action="toggle-map-layout" 
                    class="hidden md:flex items-center px-3 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-md text-sm font-bold transition border border-indigo-200"
                    title="Sembunyikan/Tampilkan Peta">
                <i class="fas fa-map-marked-alt mr-1.5 icon-map"></i>
                <span class="text-btn">Hide Map</span>
            </button>
                                                                            ...
                                                                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                                                                @foreach($customers as $c)
                                                                                    ...
                                                                                    <tr class="draggable-idpel hover:bg-yellow-50 dark:hover:bg-gray-700 transition cursor-move group" 
                                                                                        ...
                                                                                        <td class="py-1 pl-1 w-4 text-center text-gray-300 group-hover:text-indigo-400"><i class="fas fa-grip-vertical text-[8px]"></i></td>
                                                                                        <td class="py-1 w-8 font-mono font-bold text-indigo-600">{{ $seq }}</td>
                                                                                        <td class="py-1 px-1 font-bold text-gray-700 dark:text-gray-200">{{ $c->idpel }}</td>
                                                                                        <td class="py-1 px-1 truncate max-w-[100px]" title="{{ $c->nomor_meter_kwh }}">{{ Str::limit($c->nomor_meter_kwh, 15) }}</td>
