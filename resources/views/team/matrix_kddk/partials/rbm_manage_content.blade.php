...
<input type="hidden" id="ctx-unit-prefix" value="{{ $basePrefix }}">

{{-- WRAPPER UTAMA (WORKSPACE) --}}
<div id="rbm-workspace"
    class="flex flex-col h-[calc(100vh-80px)] bg-gray-100 dark:bg-gray-900 transition-all duration-300 group/workspace">

    {{-- 1. TOP BAR (Toolbar & Info) --}}
    {{-- Ubah z-20 menjadi z-40 --}}
    <div
        class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm z-40 shrink-0 relative">
        ...
        <div class="flex items-center overflow-hidden">
            ...
            <div>
                ...
                <div class="flex items-center text-xs text-gray-500 space-x-2 mt-0.5">
                    <span
                        class="bg-blue-100 text-blue-800 px-1.5 rounded font-mono font-bold">{{ $hierarchy->parent->kddk_code ?? '?' }}</span>
                    <i class="fas fa-chevron-right text-[10px]"></i>
                    <span
                        class="bg-indigo-100 text-indigo-800 px-1.5 rounded font-mono font-bold">{{ $hierarchy->kddk_code ?? '?' }}</span>
        ...
        <div class="flex items-center gap-3 md:overflow-visible custom-scrollbar pb-1 md:pb-0 relative z-[999]">
            ...
            {{-- GROUP 1: TAMPILAN --}}
            <div
                class="flex items-center gap-1 bg-gray-50 dark:bg-gray-700/50 p-1 rounded-lg border border-gray-200 dark:border-gray-600 shrink-0">
    ...
    <div class="flex flex-1 overflow-hidden relative">
        ...
        <div id="panel-list"
            class="w-full md:w-[450px] flex flex-col bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 relative z-10 shadow-xl transition-all duration-300">

            <div
                class="p-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 relative flex gap-2 items-center">
                ...
                <div class="relative flex-1">
                    ...
                    <input type="hidden" id="api-search-customer"
                        value="{{ route('team.matrix_kddk.search_customer', ['unit' => $unitCode]) }}">
                    ...
                    <button id="clear-search-btn"
                        class="hidden absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-600 cursor-pointer">
                ...
                <div id="search-results-dropdown"
                    class="hidden absolute top-full left-0 right-0 z-50 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-xl max-h-60 overflow-y-auto rounded-b-lg">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-3">
                <form id="rbm-form" action="{{ route('team.matrix_kddk.rbm_update') }}" method="POST"
                    class="ajax-form" data-success-redirect-tab="Matrix KDDK">
                    @csrf
                    <input type="hidden" id="api-route-table"
                        value="{{ route('team.matrix_kddk.get_route_table', ['unit' => $unitCode]) }}">
                    ...
                    <input type="hidden" id="bulk-remove-route"
                        value="{{ route('team.matrix_kddk.bulk_remove') }}">
                    ...
                    <div class="space-y-4">
                        @forelse($groupedData as $areaCode => $digit6Groups)
                            ...
                            <div
                                class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-300 dark:border-gray-600 overflow-hidden area-container">
                                {{-- HEADER AREA --}}
                                <div class="bg-gray-800 text-white p-3 cursor-pointer flex justify-between items-center area-header transition hover:bg-gray-700"
                                    data-action="toggle-area-map" data-target="{{ $areaId }}"
                                    data-area-code="{{ $areaCode }}" data-display-code="{{ $displayArea }}">
                                    <div class="flex items-center">
                                        <span
                                            class="bg-indigo-500 text-white font-bold px-2 py-0.5 rounded text-xs mr-2">{{ $areaCode }}</span>
                                        <div>
                                            <span class="font-bold text-sm block">{{ $areaLabel }}</span>
                                            <span class="text-[10px] text-gray-300">{{ $totalAreaPlg }}
                                                Pelanggan</span>
                                        </div>
                                    </div>
                                ...
                                {{-- BODY AREA --}}
                                <div id="{{ $areaId }}"
                                    class="hidden p-2 space-y-2 bg-gray-50 dark:bg-gray-900/50">
                                    ...
                                    <div class="digit6-wrapper space-y-2">
                                        @foreach ($digit6Groups as $digit6 => $digit7Groups)
                                            ...
                                            <div
                                                class="border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 digit6-container">
                                                {{-- HEADER DIGIT 6 --}}
                                                <div class="p-2 bg-indigo-50 dark:bg-gray-700 cursor-pointer flex justify-between items-center digit6-header hover:bg-indigo-100 dark:hover:bg-gray-600 transition"
                                                    data-action="toggle-digit6" data-target="{{ $digit6Id }}"
                                                    data-display-code="{{ $displayDigit6 }}">
                                                    <div class="flex items-center">
                                                        <div
                                                            class="w-6 h-6 rounded-full bg-indigo-200 text-indigo-800 flex items-center justify-center text-xs font-bold mr-2 border border-indigo-300">
                                                            {{ $digit6 }}
                                                        </div>
                                                        <span
                                                            class="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase">Kelompok
                                                            Rute {{ $digit6 }}</span>
                                                    </div>
                                                    <div class="flex items-center">
                                                        <span
                                                            class="text-[12px] bg-gray-200 text-gray-800 px-1.5 rounded mr-2">{{ $totalD6Plg }}
                                                            Plg</span>
                                                        <i
                                                            class="fas fa-chevron-down text-gray-400 text-xs transition-transform duration-200 icon-chevron-d6"></i>
                                                    </div>
                                                </div>

                                                {{-- BODY DIGIT 6 --}}
                                                <div id="{{ $digit6Id }}"
                                                    class="hidden border-t border-gray-100 dark:border-gray-600">

                                                    {{-- CONTAINER GRID TARGET (Untuk Mode Luas) --}}
                                                    <div
                                                        class="routes-grid-container space-y-0 transition-all duration-300">
                                                        ...
                                                        @foreach ($digit7Groups as $digit7 => $info)
                                                            ...
                                                            <div
                                                                class="route-container border-b last:border-b-0 border-gray-100 dark:border-gray-700 h-full">
                                                                {{-- HEADER HARI BACA --}}
                                                                <div class="p-2 pl-8 pr-2 flex justify-between items-center cursor-pointer hover:bg-green-50 dark:hover:bg-gray-900 transition route-header"
                                                                    ...
                                                                    <div class="flex items-center">
                                                                        <i
                                                                            class="far fa-calendar-alt text-gray-400 mr-2 text-xs"></i>
                                                                        <span
                                                                            class="text-xs font-bold text-gray-600 dark:text-gray-300 mr-2">Hari:</span>

                                                                        <span
                                                                            class="px-2 py-0.5 rounded text-xs font-bold font-mono border {{ $colorClass }}">
                                                                            {{ $digit7 }}
                                                                        </span>
                                                                    </div>
                                                                    <div class="flex items-center">
                                                                        ...
                                                                        <span
                                                                            class="text-[9px] font-bold bg-green-100 text-green-700 px-1.5 rounded">{{ $countPelanggan }}</span>
                                                                        <i
                                                                            class="fas fa-chevron-down text-gray-300 text-xs ml-2 transition-transform duration-200 icon-chevron-sub"></i>
                                                                    </div>
                                                                </div>

                                                                {{-- BODY HARI BACA (Default: Kosong & Hidden) --}}
                                                                <div id="{{ $routeId }}"
                                                                    class="route-content hidden pl-8 pr-0 pb-0"
                                                                    data-loaded="false"
                                                                    data-area-code="{{ $areaCode }}"
                                                                    data-route-code="{{ $fullRouteCode }}">
                                                                    <div class="kddk-drop-zone p-0 relative transition-colors border-l-2 border-green-200"
                                                                        data-route-prefix="{{ $routePrefix }}">

                                                                        {{-- Toolbar Petugas --}}
                                                                        <div
                                                                            class="p-1 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 flex justify-end items-center">
                                                                            <span
                                                                                class="text-[9px] text-gray-400 mr-2">Petugas:</span>
                                                                            <select
                                                                                name="assignments[{{ $firstKddk }}]"
                                                                                class="text-[10px] py-0.5 px-2 border-gray-200 rounded bg-white focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white w-40">
                                                                                <option value="">- Pilih Petugas
                                                                                    -</option>
                                                                                @foreach ($officers as $off)
                                                                                    <option
                                                                                        value="{{ $off->id }}"
                                                                                        {{ $assignedUser == $off->id ? 'selected' : '' }}>
                                                                        ...
                                                                        {{-- Indicator Drop --}}
                                                                        <div
                                                                            class="hidden drop-indicator absolute inset-0 bg-green-100/90 text-green-800 z-20 flex items-center justify-center text-xs font-bold border-2 border-dashed border-green-500">
                                                                            <i class="fas fa-file-import mr-1"></i>
                                                                            Pindah ke Hari {{ $digit7 }}
                                                                        </div>

                                                                        {{-- TABEL KOSONG (Akan diisi AJAX) --}}
                                                                        <table
                                                                            class="min-w-full text-[10px] text-left">
                                                                            <thead
                                                                                class="bg-gray-100 dark:bg-gray-700 text-gray-500 uppercase font-bold">
                                                                                <tr>
                                                                                    <th
                                                                                        class="py-1 pl-2 w-6 text-center">
                                                                                        <input type="checkbox"
                                                                                            class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 select-all-route h-3 w-3"
                                                                                            title="Pilih Semua">
                                                                                    </th>
                                                                                    <th class="py-1 pl-1 w-4"></th>
                                                                                    <th class="py-1 w-6">No</th>
                                                                                    <th class="py-1 px-1">Pelanggan
                                                                                    </th>
                                                                                    <th class="py-1 px-1">Meter</th>
                                                                                    <th class="py-1 w-8 text-center">
                                                                                        Aksi</th>
                                                                                </tr>
                                                                            </thead>
                                                                            {{-- ID UNIK UNTUK TARGET AJAX --}}
                                                                            <tbody id="tbody-{{ $routeId }}"
        ...
        <div id="panel-map" class="flex-1 bg-gray-200 dark:bg-gray-800 relative h-full hidden md:block">
            ...
            <div id="anomaly-alert"
                class="hidden absolute top-16 left-4 z-[5000] bg-red-100/95 backdrop-blur-md border border-red-400 text-red-800 px-3 py-2 rounded-lg shadow-2xl flex items-center space-x-3 animate-bounce-in-up transition-all duration-300 ease-in-out">
                <div
                    class="bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center font-bold text-xs">
                    !</div>
                <div class="text-xs">
            ...
            <div id="map-visual-controls" class="hidden absolute top-4 right-4 z-[5000] flex flex-col space-y-2">
                ...
                <div id="panel-reorder-actions"
                    class="hidden bg-white p-3 rounded shadow-xl border border-gray-300 w-64 animate-fade-in-down">
                    ...
                    <div class="flex space-x-2">
                        <button onclick="saveVisualReorder()"
                            class="flex-1 bg-green-600 hover:bg-green-700 text-white text-xs font-bold py-2 rounded">
                            <i class="fas fa-save mr-1"></i> Simpan
                        </button>
                        <button onclick="cancelVisualReorder()"
                            class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 text-xs font-bold py-2 rounded">
                            Batal
                        </button>
                    </div>
                </div>
    ...
    <div id="remove-drop-zone"
        class="hidden fixed bottom-6 left-6 z-[1000] w-40 h-16 bg-red-100/90 backdrop-blur border-2 border-dashed border-red-400 rounded-lg flex flex-col items-center justify-center text-red-600 shadow-xl transition-all duration-300 opacity-0 transform translate-y-10 kddk-remove-zone">
        <i class="fas fa-trash-alt text-xl mb-1"></i>
        <p class="text-[9px] font-bold uppercase">Lepas = Hapus</p>
    </div>
    ...
    <div id="bulk-action-bar"
        class="hidden fixed bottom-4 left-1/2 transform -translate-x-1/2 z-[9999] bg-white dark:bg-gray-800 rounded-full shadow-2xl border border-indigo-200 dark:border-indigo-700 px-4 py-2 flex items-center space-x-3 animate-bounce-in-up">
        <span class="text-xs font-bold text-gray-700 dark:text-gray-200 whitespace-nowrap">
            <span id="bulk-count" class="text-indigo-600 text-sm">0</span> <span
                class="hidden sm:inline">Dipilih</span>
        </span>
        <div class="h-4 w-px bg-gray-300 dark:bg-gray-600"></div>
        <button type="button" onclick="window.openBulkMoveModal()"
            class="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center transition px-2 py-1 hover:bg-indigo-50 rounded">
            <i class="fas fa-exchange-alt mr-1.5"></i> Pindah
        </button>
        <button type="button" onclick="window.executeBulkRemove()"
            class="text-xs font-bold text-red-600 hover:text-red-800 flex items-center transition px-2 py-1 hover:bg-red-50 rounded">
            <i class="fas fa-trash-alt mr-1.5"></i> Hapus
        </button>
        <button type="button" onclick="window.clearBulkSelection()"
            class="text-gray-400 hover:text-gray-600 ml-1 p-1 rounded-full hover:bg-gray-100">
            <i class="fas fa-times"></i>
        </button>
    </div>
    {{-- MODAL KONFIRMASI KELUARKAN (PREMIUM) --}}
    <div id="custom-confirm-modal"
        class="fixed inset-0 z-[9999] hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 transition-opacity duration-300">
        <div
            class="relative w-full max-w-sm transform rounded-2xl bg-white dark:bg-gray-800 shadow-2xl transition-all duration-300 scale-100 animate-pop-up overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-red-500 to-pink-600"></div>
            <button type="button" id="custom-confirm-cancel-x"
                class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition">
                <i class="fas fa-times text-lg"></i>
            </button>
            <div class="p-8 text-center">
                <div
                    class="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-red-50 dark:bg-red-900/20 ring-8 ring-red-50 dark:ring-red-900/10">
                    <i class="fas fa-trash-alt text-3xl text-red-500 animate-pulse"></i>
                </div>
                <h3 id="custom-confirm-title" class="mb-2 text-xl font-extrabold text-gray-900 dark:text-white">
                    Keluarkan Pelanggan?</h3>
                <div id="custom-confirm-message"
                    class="mb-6 text-sm text-gray-500 dark:text-gray-400 leading-relaxed"></div>
                <div class="flex items-center justify-center space-x-3">
                    <button type="button" id="custom-confirm-cancel"
                        class="rounded-xl border border-gray-300 bg-white px-5 py-2.5 text-sm font-bold text-gray-700 hover:bg-gray-50 transition">Batal</button>
                    <button type="button" id="custom-confirm-ok"
                        class="rounded-xl bg-red-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-red-500/30 hover:bg-red-700 hover:-translate-y-0.5 transition-all">Ya,
                        Keluarkan</button>
                </div>
            </div>
        </div>
    ...
    <div id="modal-success-generic"
        class="fixed inset-0 z-[10000] hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 transition-opacity duration-300">
        <div
            class="relative w-full max-w-sm transform rounded-2xl bg-white dark:bg-gray-800 shadow-2xl transition-all duration-300 scale-100 animate-pop-up overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-green-400 to-emerald-600"></div>
            <div class="p-8 text-center">
                <div
                    class="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-green-50 dark:bg-green-900/20 ring-8 ring-green-50 dark:ring-green-900/10">
                ...
                {{-- ID Pesan ini wajib ada: generic-success-message --}}
                <p id="generic-success-message"
                    class="mb-8 text-sm text-gray-500 dark:text-gray-400 leading-relaxed font-medium">Aksi berhasil
                    dilakukan.</p>
                <button type="button" onclick="window.closeGenericSuccessModal()"
                    class="inline-flex w-full items-center justify-center rounded-xl bg-green-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-green-500/30 transition-all duration-200 hover:bg-green-700 hover:-translate-y-0.5">OK,
                    Mengerti</button>
            </div>
        </div>
    </div>
    ...
    <div id="modal-history"
        class="fixed inset-0 z-[3000] hidden items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 transition-opacity duration-300">
        <div
            class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col transform transition-all scale-100">

            {{-- Header Modal --}}
            <div
                class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-800 rounded-t-xl">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center">
                    <i class="fas fa-history text-indigo-500 mr-2"></i> Riwayat Aktivitas (50 Terakhir)
                </h3>
                <button onclick="document.getElementById('modal-history').classList.add('hidden')"
            ...
            {{-- Footer --}}
            <div
                class="px-6 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 rounded-b-xl flex justify-end">
                <button onclick="document.getElementById('modal-history').classList.add('hidden')"
...
<style>
    ...
    #rbm-workspace.fullscreen-mode {
        ...
        /* Lepas dari aliran dokumen normal */
        top: 0 !important;
        left: 0 !important;
        right: 0 !important;
        bottom: 0 !important;
        ...
        /* Z-Index Maksimum Browser agar selalu di paling atas */

        margin: 0 !important;
        padding: 0 !important;
        border-radius: 0 !important;
        /* Hilangkan rounded corner */
        background-color: #f3f4f6;
        /* bg-gray-100 (Light Mode) */

        transform: none !important;
        /* Reset transformasi CSS jika ada */
        overflow: hidden !important;
        /* Mencegah scrollbar ganda pada body */
    }
    ...
    #rbm-workspace.fullscreen-mode #rbm-map {
        ...
        /* Reset min-height agar fleksibel */
    }
    ...
    .animate-bounce-in-up {
        animation: bounceInUp 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;
    }
