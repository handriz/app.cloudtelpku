<div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        @if (isset($activePeriod))
            <div
                class="flex items-center space-x-2 bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1.5 rounded-full border border-indigo-100 dark:border-indigo-800">
    ...
    <table class="min-w-max w-full divide-y divide-gray-300 dark:divide-gray-700">
        ...
        <thead>
            <tr class="divide-x divide-gray-300 dark:divide-gray-600">
                ...
                <th rowspan="2" class="px-3 py-3 text-left bg-gray-100 dark:bg-gray-800 align-middle"> {{-- px-4 jadi px-3 --}}
                    <span class="text-xs font-extrabold text-gray-800 uppercase tracking-wide">Unit Layanan</span>
                </th>
                
                <th rowspan="2" class="w-14 px-1 py-3 text-center bg-gray-100 dark:bg-gray-800 align-middle"> {{-- w-16 jadi w-14 --}}
                    <span class="text-xs font-bold text-gray-800 uppercase">Kode</span>
                </th>
                
                {{-- Grouping Header --}}
                <th colspan="3" class="px-1 py-2 text-center bg-gray-200/80 dark:bg-gray-900 border-b-2 border-gray-300">
                    <span class="text-xs font-bold text-gray-800 uppercase tracking-wide">Target (DIL)</span>
                </th>
                <th colspan="3" class="px-1 py-2 text-center bg-indigo-100/50 dark:bg-indigo-900/30 border-b-2 border-indigo-300">
                    <span class="text-xs font-bold text-indigo-800 uppercase tracking-wide">Sudah KDDK</span>
                </th>
                <th colspan="3" class="px-1 py-2 text-center bg-emerald-100/50 dark:bg-emerald-900/30 border-b-2 border-emerald-300">
                    <span class="text-xs font-bold text-emerald-800 uppercase tracking-wide">Progress</span>
                </th>
                
                <th rowspan="2" class="w-14 px-2 py-3 bg-gray-100 dark:bg-gray-800 text-center align-middle"> {{-- px-4 jadi px-2 --}}
                    <span class="text-xs font-bold text-gray-600 uppercase">Aksi</span>
                </th>
            </tr>

            {{-- Sub-Header (Perkecil text jadi text-[10px] atau text-xs) --}}
            <tr class="divide-x divide-gray-200 dark:divide-gray-600 text-[11px] uppercase font-bold tracking-tight">
                {{-- Target --}}
                ...
                <tr class="group hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-pointer border-b-2 border-gray-300"
                    ...
                    <td class="px-4 py-4 whitespace-nowrap bg-gray-50">
                        <div class="flex items-center">
                            ...
                            <span class="text-xs font-extrabold text-gray-900 dark:text-white uppercase">{{ $up3Name }}</span>
                        </div>
                    </td>

                    <td class="px-2 py-4 text-center bg-gray-50 border-r border-gray-300">
                        <span class="inline-block px-2 py-1 text-xs font-bold text-gray-700 bg-white rounded border border-gray-300">
                            {{ $up3Code }}
                        </span>
                    </td>

                    {{-- Data Target --}}
                    <td class="px-2 py-4 text-center text-xs font-semibold text-gray-600 tabular-nums border-l border-gray-200">{{ number_format($up3TargetPra) }}</td>
                    <td class="px-2 py-4 text-center text-xs font-semibold text-gray-600 tabular-nums">{{ number_format($up3TargetPasca) }}</td>
                    <td class="px-2 py-4 text-center text-xs font-black text-black bg-gray-100 border-r border-gray-300 tabular-nums">
                        {{ number_format($up3TargetTotal) }}
                    </td>

                    {{-- Data Realisasi --}}
                    <td class="px-2 py-4 text-center text-xs font-bold text-indigo-700 tabular-nums">{{ number_format($up3KddkPra) }}</td>
                    <td class="px-2 py-4 text-center text-xs font-bold text-pink-600 tabular-nums">{{ number_format($up3KddkPasca) }}</td>
                    <td class="px-2 py-4 text-center text-xs font-black text-indigo-900 bg-indigo-50 border-r border-indigo-200 tabular-nums">
                        {{ number_format($up3KddkTotal) }}
                    </td>

                    {{-- Data Progress --}}
                    <td class="px-2 py-4 text-center text-xs font-medium text-gray-600 tabular-nums">{{ number_format($up3Survey) }}</td>
                    <td class="px-2 py-4 text-center text-xs font-bold text-emerald-700 tabular-nums">{{ number_format($up3Valid) }}</td>
                    <td class="px-4 py-4 align-middle">
                        <div class="flex items-center justify-end space-x-2">
                            <div class="flex-1 h-2 w-16 bg-gray-200 rounded-full overflow-hidden border border-gray-300">
                                <div class="h-full bg-blue-600 rounded-full" style="width: {{ $up3Perc }}%"></div>
                            </div>
                            <span class="text-xs font-bold text-gray-900 min-w-[40px] text-right">{{ number_format($up3Perc, 1) }}%</span>
                        </div>
                    </td>
                ...
                @foreach($ulps as $row)
                    ...
                    <tr class="hidden tree-child-{{ $uniqueId }} bg-white hover:bg-yellow-50 dark:bg-gray-800 border-b border-gray-100 transition-colors cursor-pointer"
                        ...
                        <td class="px-4 py-3 whitespace-nowrap pl-12 relative border-r-0">
                            {{-- Garis Pohon --}}
                            <div class="absolute left-6 top-0 bottom-1/2 w-4 border-l-2 border-b-2 border-gray-300 rounded-bl"></div>
                            <span class="text-xs font-bold text-gray-700 hover:text-indigo-700 transition-colors relative z-10">
                                {{ $row->unit_layanan }}
                            </span>
                        </td>
                        ...
                        {{-- Data Target --}}
                        <td class="px-2 py-3 text-center text-xs font-medium text-gray-500 tabular-nums border-l border-gray-100">{{ number_format($row->target_prabayar) }}</td>
                        <td class="px-2 py-3 text-center text-xs font-medium text-gray-500 tabular-nums">{{ number_format($row->target_pascabayar) }}</td>
                        <td class="px-2 py-3 text-center text-xs font-extrabold text-gray-800 bg-gray-50/80 border-r border-gray-200 tabular-nums">
                            {{ number_format($row->target_pelanggan) }}
                        </td>

                        {{-- Data Realisasi --}}
                        <td class="px-2 py-3 text-center text-xs font-bold text-indigo-600 tabular-nums">{{ number_format($row->sudah_kddk_prabayar) }}</td>
                        <td class="px-2 py-3 text-center text-xs font-bold text-pink-600 tabular-nums">{{ number_format($row->sudah_kddk_pascabayar) }}</td>
                        <td class="px-2 py-3 text-center text-xs font-extrabold text-indigo-800 bg-indigo-50/50 border-r border-indigo-100 tabular-nums">
                            {{ number_format($row->sudah_kddk) }}
                        </td>

                        {{-- Data Progress --}}
                        <td class="px-2 py-3 text-center text-xs text-gray-500 tabular-nums">{{ number_format($row->realisasi_survey) }}</td>
                        <td class="px-2 py-3 text-center text-xs font-bold text-emerald-600 tabular-nums">{{ number_format($row->valid) }}</td>
                        <td class="px-4 py-3 align-middle">
                            <div class="flex items-center justify-end space-x-2">
                                <div class="flex-1 h-1.5 w-16 bg-gray-100 rounded-full overflow-hidden border border-gray-200">
                                    <div class="h-full bg-indigo-500 rounded-full" style="width: {{ $percentage }}%"></div>
                                </div>
                                <span class="text-xs font-bold {{ $percentage >= 100 ? 'text-emerald-700' : 'text-gray-600' }} min-w-[35px] text-right">
                        ...
                        <td class="px-4 py-3 text-center">
                            <button type="button" data-action="manage-rbm" data-url="{{ route('team.matrix_kddk.rbm_manage', ['unit' => $row->unit_code]) }}" 
                                class="text-gray-400 hover:text-white hover:bg-indigo-600 p-1.5 rounded-full transition shadow-sm border border-transparent hover:border-indigo-700 focus:outline-none" title="Susun RBM">
