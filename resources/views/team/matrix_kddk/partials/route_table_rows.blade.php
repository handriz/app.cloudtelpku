@foreach ($data as $c)
    @php
        ...
        $hasCoord = !empty($c->latitudey) && !empty($c->longitudex) && $c->latitudey != '0' && $c->longitudex != '0';
    ...
    <tr class="draggable-idpel transition cursor-move group animate-fade-in {{ $rowClass }}" draggable="true"
        data-idpel="{{ $c->idpel }}" data-lat="{{ $c->latitudey }}" data-lng="{{ $c->longitudex }}"
        data-origin-prefix="{{ $routePrefix }}" title="{{ $rowTitle }}">

        {{-- CHECKBOX --}}
        <td class="py-1 pl-2 text-center">
            <input type="checkbox" value="{{ $c->idpel }}"
                class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 select-item-row h-3 w-3">
        </td>
        ...
        <td class="py-1 text-center">
            <button type="button"
                class="text-gray-300 hover:text-red-600 transition btn-remove-customer p-1 rounded-full hover:bg-red-50 dark:hover:bg-red-900/30"
                title="Keluarkan dari Grup" data-idpel="{{ $c->idpel }}">
