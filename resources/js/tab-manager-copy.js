...
const blueIcon = new L.Icon({
   ...
   document.addEventListener('DOMContentLoaded', function () {
    ...
    function checkAndDisplayPendingNotification() {
        if (!window.pendingNotification) return;

        const { type, message } = window.pendingNotification;
        
        // Panggil displayNotification (yang sekarang lebih pintar mencari wadah)
        // Beri sedikit jeda agar DOM benar-benar ter-render
        setTimeout(() => {
            // Panggil displayNotification yang sudah diupdate
            displayNotification(type, message); 
            // Bersihkan variabel global
            window.pendingNotification = null; 
            
        }, 50); // Jeda minimal 50ms setelah inisialisasi tab/map
    }
    ...
    function loadTabContent(tabName, url, callback = null) {
        ...
        .then(html => {
           ...
           // 4. Inisialisasi Ulang Semua Script yang Dibutuhkan (Dalam setTimeout minimal)
           setTimeout(() => {
                ...
                if (mapContainer) {
                    initializeMap(mapContainer, callback);
                } else {
                    // Jika TIDAK ADA PETA (misal di tab Dashboard), panggil callback di sini
    ...
    function activateTab(tabName, url, pushHistory = true) {
        ...
        if (activeTabButton) {
            ...
            checkAndDisplayPendingNotification();
        }
        if (activeTabContent) {
